...
let apiOrigin='';
if(config.apiBase){const url=new URL(config.apiBase);if(url.protocol!=='https:')throw Error('Production PCM requires an HTTPS API.');apiOrigin=url.origin;}
...
for(const filename of ['workspace.html','index.html']) {
  ...
  const policy="default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self' https://login.microsoftonline.com https://*.msauth.net https://identitytoolkit.googleapis.com https://securetoken.googleapis.com "+apiOrigin+"; frame-src 'self' https://login.microsoftonline.com https://hrg-pay-tracker.firebaseapp.com; object-src 'none'; base-uri 'none'; form-action 'self'";
